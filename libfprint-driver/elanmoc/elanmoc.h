...
G_DECLARE_FINAL_TYPE (FpiDeviceElanmoc, fpi_device_elanmoc, FPI,
                       DEVICE_ELANMOC, FpDevice)

#define ELAN_MOC_DRIVER_FULLNAME "Elan MOC Sensors"
#define ELAN_MOC_CMD_LEN 0x3
#define ELAN_EP_CMD_OUT (0x1 | LIBUSB_ENDPOINT_OUT)   /* 0x01 */
#define ELAN_EP_CMD_IN (0x3 | LIBUSB_ENDPOINT_IN)     /* 0x83 */
#define ELAN_EP_MOC_CMD_IN (0x4 | LIBUSB_ENDPOINT_IN) /* 0x84 blocking */
#define ELAN_EP_IMG_IN (0x2 | LIBUSB_ENDPOINT_IN)     /* 0x82 image */

#define ELAN_MOC_CMD_TIMEOUT 5000
#define ELAN_MOC_BLOCKING_TIMEOUT 10000
#define ELAN_MOC_CAL_RETRY 500
#define ELAN_MOC_ENROLL_TIMES 8
#define ELAN_MAX_USER_ID_LEN 92
#define ELAN_MAX_ENROLL_NUM 10
#define ELAN_MAX_HDR_LEN 3
#define ELAN_USERDATE_SIZE (ELAN_MAX_USER_ID_LEN + 3)
#define ELAN_FINGER_INFO_LEN 70
#define ELAN_SID_LEN 69

/* ── Response / error codes ──────────────────────────────────── */

#define ELAN_MSG_OK 0x00
#define ELAN_MSG_NOT_READY 0xFE
#define ELAN_MSG_VERIFY_ERR 0xFD
#define ELAN_MSG_HLK_FAIL 0xFC
#define ELAN_MSG_DIRTY 0xFB
#define ELAN_MSG_ERROR 0xFF
...
#define ELAN_MSG_MAX_ENROLLED 0xDD
...
/* MOC commands (prefix 0x40, 0xFF, <sub>) */
...
/* Enroll touch — 3B header + 4B payload (id, total, done, 0x00).
 * Reads from EP 0x84 (blocking). */
...
static const struct elanmoc_cmd elanmoc_abort_cmd = {
  .cmd_header = {0x40, 0xFF, 0x02},
  .cmd_len = 3,
  .resp_len = 0,
};

/* Verify — reads from EP 0x84 (blocking). */
static const struct elanmoc_cmd elanmoc_verify_cmd = {
  .cmd_header = {0x40, 0xFF, 0x03},
  .cmd_len = 3,
  .resp_len = 2,
};

/* Enrollment collision / duplicate check */
static const struct elanmoc_cmd elanmoc_enroll_collision_cmd = {
  .cmd_header = {0x40, 0xFF, 0x10},
  .cmd_len = 3,
  .resp_len = 3,
};

/* Commit enrollment with SID (3B header + 69B SID) */
static const struct elanmoc_cmd elanmoc_enroll_commit_cmd = {
  .cmd_header = {0x40, 0xFF, 0x11},
  .cmd_len = 3 + ELAN_SID_LEN,
  .resp_len = 2,
};

/* Delete finger by SID (3B header + 69B SID) */
...
/* Set sensor mode — 3B header + 1B mode (0=normal, 1=VBS) */
...
/* Bulk-wipe all fingerprints. */
static const struct elanmoc_cmd elanmoc_remove_all_cmd = {
  .cmd_header = {0x40, 0xFF, 0x99},
  .cmd_len = 3,
  .resp_len = 0,
};

static const struct elanmoc_cmd elanmoc_finger_info_cmd = {
  .cmd_header = {0x40, 0xFF, 0x12},
  .cmd_len = 4,
  .resp_len = ELAN_FINGER_INFO_LEN,
};
...
struct _FpiDeviceElanmoc
{
  ...
  unsigned char   x_trace;   /* sensor width  (raw + 1) */
  unsigned char   y_trace;   /* sensor height (raw + 1) */
