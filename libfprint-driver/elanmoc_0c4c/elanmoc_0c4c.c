...
static void
elanmoc_cmd_ack_cb (FpiDeviceElanmoc0c4c *self,
                    uint8_t          *buffer_in,
                    gsize             length_in,
                    GError           *error)
{
...
static void
enroll_status_report (FpiDeviceElanmoc0c4c *self, int enroll_status_id,
...
static void
elanmoc_enroll_cb (FpiDeviceElanmoc0c4c *self,
                   uint8_t          *buffer_in,
                   gsize             length_in,
                   GError           *error)
{
 ...
 *
 * Response layout (3 bytes):
 *   [0] = 0x40  (ACK prefix, same as all MOC responses)
 *   [1] = status
 *         0x00 → no collision, safe to commit
 *         non-zero → duplicate finger already enrolled at slot [2]
 *   [2] = slot index of the colliding print (valid when [1] != 0x00)
 *
 * On collision we fail enrollment with FP_DEVICE_ERROR_DATA_DUPLICATE so that
 * libfprint / the caller can surface a meaningful error to the user instead of
 * silently writing a second copy of the same finger.
 */
static void
                             ...
                             GError           *error)
{
  ...
  if (buffer_in[1] != ELAN_MSG_OK)
    {
      /* buffer_in[2] holds the conflicting slot index when available */
...
static void
elanmoc_commit_cb (FpiDeviceElanmoc0c4c *self,
                   uint8_t          *buffer_in,
                   gsize             length_in,
                   GError           *error)
{
 ...
 *
 * Updated flow:
 *   MOC_ENROLL_GET_ENROLLED_NUM  – query how many fingers are stored
 *   MOC_ENROLL_REENROLL_CHECK    – check if this user is re-enrolling
 *   MOC_ENROLL_WAIT_FINGER       – capture touches (loops until complete)
 *   MOC_ENROLL_CHECK_COLLISION   – NEW: verify the captured template is not
 *                                  a duplicate of an existing finger
 *   MOC_ENROLL_COMMIT_RESULT     – persist the new template on the sensor
 */
static void
elan_enroll_run_state (FpiSsm *ssm, FpDevice *dev)
{
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    /* NEW: send the collision / duplicate-detection command after all touches
     * have been captured but before the template is committed to flash.
     * elanmoc_enroll_collision_cmd has resp_len=3 (ACK, status, slot). */
    case MOC_ENROLL_CHECK_COLLISION:
      cmd_buf = elanmoc_compose_cmd (&elanmoc_enroll_collision_cmd);
      elanmoc_get_cmd (dev, cmd_buf,
                       elanmoc_enroll_collision_cmd.cmd_len,
                       elanmoc_enroll_collision_cmd.resp_len,
                       0,
                             ...
                             GError           *error)
{
  ...
  /* Slot is empty — sensor returns 0xFE in byte 1 */
...
elan_list_run_state (FpiSsm *ssm, FpDevice *dev)
{
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case MOC_LIST_GET_FINGER:
        ...
        elanmoc_get_cmd (dev, cmd_buf,
                         elanmoc_finger_info_cmd.cmd_len,
                         elanmoc_finger_info_cmd.resp_len,
                         0,
  ...
  if (fpi_device_get_current_action (device) == FPI_DEVICE_ACTION_IDENTIFY)
    {
      ...
      found = g_ptr_array_find_with_equal_func (prints, print,
...
static void
identify_status_report (FpiDeviceElanmoc0c4c *self, int verify_status_id,
                        int data, GError *error)
{
  FpDevice *device = FP_DEVICE (self);
  guint8 *cmd_buf = NULL;
      ...
      case RSP_VERIFY_OK:
        {
          ...
          cmd_buf = elanmoc_compose_cmd (&elanmoc_finger_info_cmd);
          cmd_buf[3] = data;
          elanmoc_get_cmd (device, cmd_buf,
                           elanmoc_finger_info_cmd.cmd_len,
                           elanmoc_finger_info_cmd.resp_len,
                           0,
...
static void
elanmoc_identify_cb (FpiDeviceElanmoc0c4c *self,
                     uint8_t          *buffer_in,
                     gsize             length_in,
                     GError           *error)
{
...
static void
elanmoc_delete_cb (FpiDeviceElanmoc0c4c *self,
                   uint8_t          *buffer_in,
                   gsize             length_in,
                   GError           *error)
{
...
/* ── Clear storage (bulk wipe) ───────────────────────────────────────────────
 *
 * elanmoc_remove_all_cmd (0x40 0xFF 0x99) has resp_len=0, meaning the sensor
 * sends no data back — it just executes the wipe.  We therefore skip the GET
 * phase entirely (cmd_len_in == 0 causes fp_cmd_run_state to call the callback
 * with a NULL buffer and mark the SSM completed immediately).
 *
 * State machine: single state CLEAR_SEND_CMD → SSM completes → done callback.
 */
                          ...
                          GError           *error)
{
  ...
  /* resp_len is 0 for remove_all — a NULL/empty response is the success case */
  fp_info ("elanmoc clear storage complete");
  fpi_ssm_next_state (self->task_ssm);
}

static void
task_ssm_clear_storage_done (FpiSsm *ssm, FpDevice *dev, GError *error)
{
  FpiDeviceElanmoc0c4c *self = FPI_DEVICE_ELANMOC_0C4C (dev);
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case CLEAR_SEND_CMD:
      ...
      elanmoc_get_cmd (dev, cmd_buf,
                       ...
                       elanmoc_remove_all_cmd.resp_len,   /* 0 — no response expected */
                       0,
...
static void
elanmoc_cmd_ver_cb (FpiDeviceElanmoc0c4c *self,
                    uint8_t          *buffer_in,
                    gsize             length_in,
                    GError           *error)
{
...
static void
elanmoc_cmd_dim_cb (FpiDeviceElanmoc0c4c *self,
                    uint8_t          *buffer_in,
                    gsize             length_in,
                    GError           *error)
{
...
dev_exit_handler (FpiSsm *ssm, FpDevice *dev)
{
  ...
  switch (fpi_ssm_get_cur_state (ssm))
    ...
    case DEV_EXIT_ABORT:
      ...
      elanmoc_get_cmd (dev, cmd_buf, elanmoc_abort_cmd.cmd_len, elanmoc_abort_cmd.resp_len, 0, elanmoc_cmd_ack_cb);
      break;
    }
}
